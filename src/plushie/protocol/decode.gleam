...
import plushie/event/types.{
  type EventTarget, type KeyLocation, type Modifiers, type MouseButton,
  type PointerType, BackButton, EffectCancelled, EffectError, EffectOk,
  EffectUnsupported, EventTarget, ForwardButton, LeftButton, LeftSide, Line,
  MiddleButton, Mouse, Numpad, OtherButton, Pixel, RightButton, RightSide,
  Standard, Touch,
}
...
pub type InboundMessage {
  ...
  EffectResponseRaw(wire_id: String, result: types.EffectResult)
...
fn decode_widget_click(
  ...
  Ok(EventMessage(event.WidgetClick(target:)))
}

fn decode_widget_string_value(
  map: Dict(String, PropValue),
  constructor: fn(EventTarget, String) -> Event,
) -> Result(InboundMessage, protocol.DecodeError) {
  use target <- result.try(decode_windowed_target(map))
  use value <- result.try(get_string(map, "value"))
  Ok(EventMessage(constructor(target, value)))
}
...
fn decode_widget_toggle(
  ...
  Ok(EventMessage(event.WidgetToggle(target:, value:)))
}

fn decode_widget_float_value(
  map: Dict(String, PropValue),
  constructor: fn(EventTarget, Float) -> Event,
) -> Result(InboundMessage, protocol.DecodeError) {
  use target <- result.try(decode_windowed_target(map))
...
fn decode_widget_no_value(
  map: Dict(String, PropValue),
  constructor: fn(EventTarget) -> Event,
) -> Result(InboundMessage, protocol.DecodeError) {
  use target <- result.try(decode_windowed_target(map))
  Ok(EventMessage(constructor(target)))
}
...
fn decode_widget_key_binding(
  ...
  Ok(EventMessage(event.WidgetKeyBinding(target:, value: binding)))
}
...
fn decode_widget_scrolled(
  ...
  Ok(EventMessage(event.WidgetScrolled(target:, data: scroll_data)))
}
...
fn decode_generic_widget_event(
  ...
  Ok(EventMessage(event.WidgetEvent(kind: family, target:, value:, data:)))
}
...
fn decode_sub_cursor_entered(
  ...
  let wid = get_string_or(map, "window_id", "")
  let target = EventTarget(window_id: wid, id: wid, scope: [])
  Ok(EventMessage(event.WidgetEnter(target:)))
}
...
fn decode_sub_cursor_left(
  ...
  let wid = get_string_or(map, "window_id", "")
  let target = EventTarget(window_id: wid, id: wid, scope: [])
  Ok(EventMessage(event.WidgetExit(target:)))
}
...
fn decode_pane_resized(
  ...
  Ok(EventMessage(event.PaneResized(target:, split:, ratio:)))
}
...
fn decode_pane_simple(
  ...
  Ok(EventMessage(constructor(target, pane)))
}
