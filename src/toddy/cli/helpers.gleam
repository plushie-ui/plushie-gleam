...
/// Look up the toddy source checkout path from the TODDY_SOURCE_PATH
/// environment variable.
pub fn source_path() -> Result(String, Nil) {
  ffi.get_env("TODDY_SOURCE_PATH")
}

/// Error when the toddy binary cannot be resolved.
...
pub fn resolve_error_message(err: ResolveError) -> String {
  case err {
    BinaryNotFound(_) ->
      "toddy binary not found. Run bin/toddy_download or bin/toddy_build, "
      <> "or set TODDY_BINARY_PATH."
    ExplicitPathMissing(path:) ->
      "toddy binary not found at explicit path: " <> path
  }
}
