...
put_pool_session(Pool, SessionId) ->
    put(toddy_pool_session, {Pool, SessionId}),
    nil.

get_pool_session() ->
    case get(toddy_pool_session) of
        undefined -> {error, nil};
        {Pool, SessionId} -> {ok, {Pool, SessionId}}
    end.

erase_pool_session() ->
    erase(toddy_pool_session),
    nil.
...
collect_interact_events(Timeout, Acc) ->
    receive
        ...
        {toddy_pool_event, _SessionId, #{<<"type">> := <<"interact_step">>, <<"events">> := Events}} ->
            collect_interact_events(Timeout, Acc ++ Events);
        {toddy_pool_event, _SessionId, #{<<"type">> := <<"interact_response">>, <<"events">> := Events}} ->
            Acc ++ Events
    after Timeout ->
        Acc
    end.
