...
pub fn layer_shapes_auto_id_includes_layer_name_test() {
  let node =
  ...
  assert shape_node.id == "auto:bg/shape_0"
}

pub fn flat_shapes_auto_id_uses_default_prefix_test() {
  let node =
    canvas.new("c", length.Fixed(100.0), length.Fixed(100.0))
    |> canvas.shapes([
  ...
  assert s0.id == "auto:default/shape_0"
  assert s1.id == "auto:default/shape_1"
}
